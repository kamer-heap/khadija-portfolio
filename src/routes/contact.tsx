...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Let's talk about backend, systems, AI, or collaboration." },
    ],
  }),
...
function ContactPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 sm:px-6 pt-28 sm:pt-32 pb-20 overflow-x-hidden">
      <header className="max-w-3xl">
        <p className="text-xs font-mono text-primary uppercase tracking-[0.3em]">/ contact</p>
        <h1 className="mt-3 text-3xl sm:text-5xl font-bold tracking-tight text-gradient">Let's build something</h1>
        <p className="mt-6 text-base sm:text-lg text-muted-foreground">
          Open to collaboration on backend systems, databases, intelligent applications, and CS community initiatives.
          Drop a note or reach out on any channel below.
        </p>
      </header>

      <div className="mt-12 sm:mt-16 grid lg:grid-cols-5 gap-6 sm:gap-8">
        {/* Form */}
        <form onSubmit={onSubmit} className="lg:col-span-3 glass rounded-2xl p-5 sm:p-8 space-y-5">
        ...
        <aside className="lg:col-span-2 space-y-4">
          <a
            href={`mailto:${email}`}
            className="glass glow-border rounded-2xl p-4 sm:p-5 flex items-center gap-3 sm:gap-4 group"
          >
            <div className="h-11 w-11 shrink-0 grid place-items-center rounded-lg bg-primary/10 text-primary border border-primary/30 group-hover:bg-primary/20 transition">
              <Mail size={18} />
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-xs font-mono text-muted-foreground uppercase tracking-widest">Email</p>
              <p className="font-display font-semibold truncate text-sm sm:text-base">{email}</p>
            </div>
            <button
              type="button"
              onClick={(e) => { e.preventDefault(); copyEmail(); }}
              className="shrink-0 text-xs font-mono px-2 py-1 rounded bg-secondary/60 hover:bg-primary/20 hover:text-primary transition"
          ...
          <a
            href="https://github.com/kamer-heap"
            target="_blank"
            rel="noreferrer"
            className="glass glow-border rounded-2xl p-4 sm:p-5 flex items-center gap-3 sm:gap-4 group"
          >
            ...
            <div className="min-w-0">
              ...
              <p className="font-display font-semibold truncate text-sm sm:text-base">github.com/kamer-heap</p>
            </div>
          </a>
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <p className="font-display font-semibold truncate text-sm sm:text-base">/in/khadija-amer</p>
            </div>
          </a>
        </aside>
      </div>
